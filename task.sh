...
function processDataFile_for_condition() {
  ...
  do
    if filterCondition $Status ; then
      check_data $URL $Module $FullyTestName
    fi
  done < $File
}
...
function check_data() {
  ...
  log_test_info $URL $module $testClass $testMethod
}
...
function log_test_info() {
  ...
  test_info_for_commit $testMethod $TestClass $module | tee -a $resultFile
  back
}
...
# paramters in order: test name, test_file, module
function test_info_for_commit() {
  ...
  if ! test_exists $TestClass $testMethod $module java && ! test_exists $TestClass $testMethod $module scala && ! test_exists $TestClass $testMethod $module groovy; then
    echo Test $testClass $testMethod does NOT exist in the latest commit 
  #  else
...
# parameters in order: testClass, testMethod, module, file format
